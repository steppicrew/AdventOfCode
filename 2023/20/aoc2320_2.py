...
def run() -> int:
    ...
    def get_cycle_counts(names: set[str] | list[str]) -> dict[str, int]:
        ...
        states: dict[
            str,
            set[tuple[FrozenConjunctionStates, FrozenFlipFlopStates]]
        ] = {
        ...
        while len(names) > len(counts):
            ...
            for name in names:
                if name in counts:
                    continue
                new_state = freeze_state(sources[name])
    ...
    counts = get_cycle_counts(
        module_sources[module_sources["rx"][0]]
        if "rx" in module_sources
        else list(modules.keys())
    )
