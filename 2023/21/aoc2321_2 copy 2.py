...
def run() -> int:
    ...
    if STEP == 1:
        ...
        full_field_count = field_counts["middle-east"][0]

        middle_index = STEP_COUNT % 2
        not_middle_index = middle_index if width % 2 == 0 else 1 - middle_index

        center_index = not_middle_index
        next_to_center_index = (STEP_COUNT - offset - 1) % 2
        alternate_index = next_to_center_index if width % 2 == 0 else 1 - next_to_center_index

        middle_outer_index = next_to_center_index if full_radius % 2 == 0 else 1 - \
            next_to_center_index
        middle_not_outer_index = 1 - middle_outer_index

        corner_outer_index = middle_outer_index if offset % 2 == 0 else 1 - middle_outer_index

        corner_radius = full_radius_mod + offset
        # next_corner_radius = full_radius_mod - offset - 1
        next_corner_radius = corner_radius - width + 1

        count1 = (full_radius + 1) * (full_radius + 1)
        count2 = full_radius * full_radius

        if center_index == middle_not_outer_index:
            count1 += 1
            count2 -= 1

        # all full fields
        result = count1 * full_field_count[middle_not_outer_index] + \
            count2 * full_field_count[middle_outer_index]
        ...
        debug("outer_index", middle_outer_index)
        debug("full_radius", full_radius, full_radius_mod)
        debug("full_field_count odd", full_field_count[1])
        debug("full_field_count even", full_field_count[0])
        ...
        north_east_count = len(
            [v for v, step in field_counts["north-east"][1].items() if step <= corner_radius and step % 2 == corner_outer_index])
        north_west_count = len(
            [v for v, step in field_counts["north-west"][1].items() if step <= corner_radius and step % 2 == corner_outer_index])
        south_east_count = len(
            [v for v, step in field_counts["south-east"][1].items() if step <= corner_radius and step % 2 == corner_outer_index])
        south_west_count = len(
            [v for v, step in field_counts["south-west"][1].items() if step <= corner_radius and step % 2 != corner_outer_index])

        next_north_east_count = len(
            [v for v, step in field_counts["north-east"][1].items() if step <= next_corner_radius and step % 2 != corner_outer_index])
        next_north_west_count = len(
            [v for v, step in field_counts["north-west"][1].items() if step <= next_corner_radius and step % 2 != corner_outer_index])
        next_south_east_count = len(
            [v for v, step in field_counts["south-east"][1].items() if step <= next_corner_radius and step % 2 != corner_outer_index])
        next_south_west_count = len(
            [v for v, step in field_counts["south-west"][1].items() if step <= next_corner_radius and step % 2 != corner_outer_index])

        result += north_count + east_count + west_count + south_east_count
