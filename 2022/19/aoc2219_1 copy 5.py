...
def run():
    ...
    def get_best_robots_to_build(inventory: Inventory, robots: Robots, blueprint: Blueprint, time_left: int) -> set[Robots]:
        def better_than_none(needed_ore, secondary: int, secondary_one: Robots) -> set[Robots]:
            result: set[Robots] = {NONE}
            if can_build(inventory, blueprint[secondary]) and blueprint[secondary][ORE] + needed_ore <= robots[ORE]:
                result.add(secondary_one)
            if can_build(inventory, blueprint[ORE]) and blueprint[ORE][ORE] + needed_ore <= robots[ORE]:
                result.add(ONE_ORE)
            return result
        ...
        if geode_time_until_obsidian <= MIN_TIME and geode_time_until_ore <= MIN_TIME:
            return better_than_none(geode_needs_ore, secondary=OBSIDIAN, secondary_one=ONE_OBSIDIAN)
        ...
        if geode_time_until_obsidian > geode_time_until_ore:
            ...
            if obsidian_time_until_clay <= MIN_TIME and obsidian_time_until_ore <= MIN_TIME:
                return better_than_none(obsidian_needs_ore, secondary=CLAY, secondary_one=ONE_CLAY)
