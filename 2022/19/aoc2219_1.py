...
def run():
    ...
    def need_clay(clay_needed: int, ore_count: int, ore_robots: int, clay_robots: int, ore_robot_price: int, clay_robot_price: int, max_ore_robots: int) -> tuple[int, set[Robots]]:

        # if we reach our limit next time do nothing
        if clay_robots >= clay_needed:
            return (1 + ceil((max_ore_robots - ore_robots)/ore_robot_price), {NONE})

        results: set[tuple[int, Robots]] = set()

        # Variant 1: do nothing and wait until my clay robots have harvested enough clay
        if clay_robots:
            results.add((ceil(clay_needed/clay_robots) +
                        ceil((max_ore_robots - ore_robots)/ore_robot_price), NONE))
        ...
        if geode_time_until_obsidian > geode_time_until_ore:
            ...
            if obsidian_time_until_clay > obsidian_time_until_ore:
                best_option = need_clay(
                    ...
                    max_ore_robots=max(price[ORE] for price in blueprint)
                )
