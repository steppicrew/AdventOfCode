...
def run():
    ...
    def collect(robots: Robots) -> Inventory:
        return Inventory(
            ore=robots.ore,
            clay=robots.clay,
    ...
    def time_until_robot(offset_inventory: Inventory, diff_inventory: Inventory, cost: Cost):
        max_time = ceil((cost.ore-offset_inventory.ore) / diff_inventory.ore)

        if cost.obsidian:
            if not diff_inventory.obsidian:
                return 0
            max_time = max(max_time, ceil(
                (cost.obsidian-offset_inventory.obsidian) / diff_inventory.obsidian))

        if cost.clay:
            if not diff_inventory.clay:
                return 0
            max_time = max(max_time, ceil(
                (cost.clay-offset_inventory.clay) / diff_inventory.clay))
    ...
    def add_inventory(inventory: Inventory, add: Inventory, count: int = 1) -> Inventory:
        return Inventory(
            ore=inventory.ore + count * add.ore,
            obsidian=inventory.obsidian + count * add.obsidian,
            clay=inventory.clay + count * add.clay,
            geode=inventory.geode + count * add.geode,
        )
    ...
    def minute(time: int, robots: Robots, inventory: Inventory, blueprint: Blueprint) -> int:
        ...
        diff_inventory = collect(robots)
        ore_time = time_until_robot(
            inventory, diff_inventory, blueprint.ore_robot)
        obsidian_time = time_until_robot(
            inventory, diff_inventory, blueprint.obsidian_robot)
        clay_time = time_until_robot(
            inventory, diff_inventory, blueprint.clay_robot)
        geode_time = time_until_robot(
            inventory, diff_inventory, blueprint.geode_robot)

        times = sorted(
            list(set(
                filter(
                    lambda t: t > 0 and time + t < MAX_TIME,
                    (ore_time, obsidian_time, clay_time, geode_time)
                )
            ))
        )
        max_time = max(times)
        ...
        geode_count = 0
        if geode_time:
            next_geode_inventory = sub_cost(
                add_inventory(inventory, diff_inventory, geode_time),
                blueprint.geode_robot
            )
            geode_count = max(geode_count, minute(
                time + geode_time,
                add_robots(robots, Robots(geode=1)),
                ,
                blueprint
            ))
        ...
        if obsidian_time:
            geode_count = max(geode_count, minute(
                time + obsidian_time,
                add_robots(robots, Robots(obsidian=1)),
                sub_cost(
                    add_inventory(inventory, diff_inventory, obsidian_time),
        ...
        if clay_time:
            geode_count = max(geode_count, minute(
                time + clay_time,
                add_robots(robots, Robots(clay=1)),
                sub_cost(
                    add_inventory(inventory, diff_inventory, clay_time),
        ...
        if ore_time:
            geode_count = max(geode_count, minute(
                time + ore_time,
                add_robots(robots, Robots(ore=1)),
                sub_cost(
                    add_inventory(inventory, diff_inventory, ore_time),
                    blueprint.ore_robot
                ),
                blueprint
            ))
